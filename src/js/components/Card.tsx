...
function Card() {
  ...
  const [cardRequest, setCardRequest] = React.useState<CardRequest|null|undefined>()
  ...
  return (
    <SessionContext.Consumer>
      {({ user }) => (
        <Page title={(user!.rfid || cardRequest) && "Card"} sidebar="true" icon={<CreditCardIcon />}>
          {user!.rfid ? (
            <CardBox
              card={<RfidCard number={user!.rfid} simulate={false} />}
              details={['Requested At: --/--', 'Expires At: --/--']}
              controls={[
                <Button variant="light" color="primary" onClick={requestCard} loading={isLoading}>
                  Request New Card
                </Button>,
            ...
            <CardBox
              ...
              details={[
                `Request At: ${datetime(cardRequest?.requestedAt)}`
              ]}
              controls={[
                <Alert variant='subtle' color="warning">Pending review. You will be notified of an answer as soon as possible.</Alert>,
              ]}/>
          ) : (
            <PlaceholderScreen
              icon={<CardVector />}
